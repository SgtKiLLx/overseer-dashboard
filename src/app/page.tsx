...
export default async function AdminDashboard({
  ...
  // Fetch Live Data (Filtered by your Server ID)
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-slate-300 font-sans selection:bg-cyan-500/30 flex flex-col lg:flex-row">
      ...
      {/* 2. MOBILE BOTTOM NAV (FULL SECTION GLOW) */}
      <div className="lg:hidden fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-[92%] max-w-md">
                        ...
                        <tbody className="divide-y divide-white/[0.02]">
                            {registrations.map(reg => (
                                <tr key={reg.id} className="group hover:bg-white/[0.01]">
                                    ...
                                    <td className="p-8"><p className="text-sm font-semibold">{reg.ign}</p><p className="text-[10px] text-cyan-900 font-mono uppercase font-bold">{reg.xboxGamertag}</p></td>
                                    <td className="p-8 text-right pr-10">
                                        <form action={wipeSurvivor}><input type="hidden" name="id" value={reg.id} /><button className="p-4 rounded-2xl bg-red-500/[0.02] text-red-500/40 hover:bg-red-500 hover:text-white transition-all transform active:scale-90"><Trash2 size={18} /></button></form>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
           ...
           <div className="space-y-8 animate-in fade-in duration-500">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {alphaClaims.length === 0 && (
                   <div className="md:col-span-2 bg-white/[0.02] border border-dashed border-white/10 rounded-[48px] p-20 text-center text-slate-700 uppercase font-black text-xs tracking-widest italic">No pending requests</div>
                )}
                {alphaClaims.map(claim => (
           ...
           <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <form action={updateConfig} className="bg-[#050505] border border-white/[0.05] rounded-[48px] p-8 md:p-16 shadow-2xl space-y-12">
                ...
                <div className="pt-10 border-t border-white/[0.03] flex justify-end">
                    <button type="submit" className="bg-white text-black font-[1000] px-12 py-5 rounded-[28px] hover:bg-cyan-400 transition-all text-xs uppercase italic flex items-center gap-3 shadow-xl"><Save size={18} /> Sync Protocols</button>
                </div>
             </form>
           </div>
        )}
